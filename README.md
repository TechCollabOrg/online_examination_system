...
在「题库管理 → 添加/编辑试题」中，除题干文字与图片外，可使用 **「试题音频」** 上传听力材料（支持 MP3、WAV、M4A、OGG、AAC，单文件不超过 50MB，默认最多 3 段）。多条音频 URL 在库中用 **###** 拼接，规则与试题图片相同。

学生端在**考试作答**、**考后解析**、**阅卷**、**刷题记录**、**错题本**等页面，题干下方会显示音频播放器，可反复播放。

**已有数据库**：若 `t_question` 尚无 `audio` 列，请在 MySQL 中执行一次 `online-exam-system-backend/sql/alter_t_question_audio.sql`。全新按 `db_exam.sql` 建库时已包含该列。
...
- **注册页**（`/register`）可选择身份：**学生**、**教师**、**管理员**。
- **学生**可直接注册，无需邀请码，须填写**专业**。
- **教师**、**管理员**注册须填写管理员发放的**邀请码**（码与身份须一致，且未过期、未用尽）。
- **仅管理员**可在侧栏「**邀请码管理**」中生成、禁用、删除邀请码，并设置可用次数与过期时间。

**已有数据库**：在 MySQL 中执行一次 `online-exam-system-backend/sql/create_t_invite_code.sql`。
...
| 功能   | 说明                |
| ---- | ----------------- |
| 邀请码管理 | 生成教师/管理员注册邀请码，设置次数与有效期 |
| 用户管理 | 创建/停用账号，分配学生或教师角色；学生须填写专业；可编辑学生班级与专业 |
| 班级管理 | 创建班级，批量导入学生       |
| 日志查看 | 查看所有操作记录          |
| 考试管理 | 对所有考试有完整管理权限      |
| 统计报表 | 全站数据概览            |
...
>
> 正式上线前请及时修改以上密码。  
> 新用户也可打开 `http://localhost:9527/#/register` 自助注册：学生无需邀请码；教师/管理员须由现有管理员在「邀请码管理」中生成邀请码后再注册。
...
- 若 `url` 是 `/api/notices/new`、`/api/stat/student` 等**首页统计接口**，常见原因是浏览器里 **`localStorage.roles` 与当前 JWT 角色不一致**（例如上次是学生、这次用教师登录未清缓存）。处理：**F12 → Application → 清除本站 Cookie / sessionStorage / localStorage**，刷新后重新登录；当前版本已在路由守卫中自动丢弃过期 JWT，首页也改为以 JWT 的 `roleId` 决定展示哪个角色面板
...
本系统里**没有**再点一次「发布考试」的步骤：教师在**创建考试时**选择发布范围并保存成功后，考试会出现在对应学生的**试卷中心**里。

- **按班级**：勾选「考试班级」，该班全体学生可见。
- **按学生**：切换为「按学生」，可先按班级筛选，再勾选多名学生（可跨多个班级）；仅被选中的学生可见。

学生操作路径：**用学生账号登录** → 左侧菜单 **试卷中心** → 列表里在考试开始与结束时间之间会显示 **开始考试** → 进入 **准备考试** 页再点开始答题。

注意：按班级发布时，学生须在对应班级；按学生发布时，须被教师勾选。当前时间须在考试的 **开始时间～结束时间** 内，否则按钮会是「未开始」或「已结束」且不可点。
...
- 配置完成后点击 **「生成试卷题目」**：系统会按规则随机抽题并展示在下方列表中，可 **查看详情**、**设置分值**（支持两位小数，如 `2.5`）、**移除** 某题，或通过 **「从题库添加」** 补充题目；不满意可点 **「重新随机抽题」**
- **必须先预览并确认题目后再保存**；保存时以列表中的题目与分值为准，不会在保存时再次悄悄换题
- 试卷总分会按已确认题目的分值相加自动计算；若尚未生成预览，则按「各题型数量 × 统一分值」估算
- 若一直为 0，请确认每类题「数量与分值都大于 0」且已选择题库
...
**复合题判分**：仅含单选/多选/判断小问的复合题，交卷后按整题自动判对错；若同时含简答子题，客观小题会在交卷时按答对比例先计入客观分，简答部分由教师批改或 **AI 阅卷** 后确认总分。
...
*最后更新：2026-07-02* · 监考详细配置见 [docs/PROCTOR_SETUP.md](docs/PROCTOR_SETUP.md)
