...
在「题库管理 → 添加/编辑试题」中，**题干**与**客观题各选项**可单独上传多张附图（上传组件默认最多 **500** 张/处）。多张 URL 在库中用 **###**（三个半角井号）拼成一条字符串；旧数据单 URL 不受影响。若 Excel 导入一格多图，可用 **###** 连接多个地址（常由导题脚本生成）。**简答题参考答案**不在此列，而在下方「参考答案（图文）」富文本中内嵌图片（见下段）。
...
1. 在题库中先建一条「**仅作材料用**」的父题（`t_question` 一行）：把共用文字、示意图放在该行的 `content`、`image` 里；该行**不必**加入某场考试的抽题列表（考试里只加小题即可）。
...
**Excel 批量导入（推荐）**：模板 `public/template/ImportQuestionTemplate.xlsx` 末尾有三列（第 23～25 列）：**材料组编号**、**共用材料题干**、**共用材料题干图片**。同一材料下的多道小题填写**相同**的材料组编号（如 `G001`）；**该组第一行**必须填写「共用材料题干」（整段材料文字），「共用材料题干图片」可填可不填；**每一行**的「试题类型」「题干」「各选项」仍按**单个小问**填写——题干里写 `(1)…`、`(2)…` 等即可；从第二行起「共用材料题干」可留空。导入时会自动插入一条「父题」并给各小题写好 `parent_qu_id`（父题为简答题占位，**组卷时不要勾选父题**，只勾选各小题）。

**教师端手动录入**：在「新增试题」页填写 **共用材料父题 ID**（可先在列表里看材料简答题的 **ID** 列，或点该行 **材料下小问** 自动带上）；题目内容只写本小问；客观题选项不够点表格 **添加** 增加行。保存后可点 **保存并继续添加同材料下一小问** 连续录多小问。
...
**自己选题（组卷信息第一个页签）**：不必先在「随机抽题」里绑定题库 ID；保存时后端会根据已选试题自动写入关联题库。若提示「所选题目未关联题库」，请在试题管理中给相关题目指定所属题库后再保存。
